...
const ModalEditar = ({ setModalVisible, modalVisible, idDetalle, setCantidadProductoCarrito, cantidadProductoCarrito, getDetalleCarrito, existencias }) => {
  //Url de la api
  ...
  const handleUpdateDetalleCarrito = async () => {
    ...
    try {
      if (cantidadProductoCarrito <= 0) {
      ...
      } else if (cantidadProductoCarrito > existencias) {
        setAlertType(2);
        setAlertMessage(`La cantidad solicitada no puede superar las existencias actuales ${existencias}`);
        setAlertCallback(null);
        setAlertVisible(true);
        return; // Corrige la lógica aquí
      } else {
        ...
        if (data.status) {
          ...
          setAlertType(2);
          setAlertMessage(`Error al editar detalle carrito ${data.error}`);
          setAlertCallback(null);
          setAlertVisible(true);
        }
